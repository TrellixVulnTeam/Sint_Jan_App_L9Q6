...
async function printscedule() {//print scedule to html document
	...
	for (var i = 0; i < scedule.length; i++) {//for every scedule entry
		...
		var endTime = new Date(scedule[i].eindDatumTijd);//gets the end time of the subject
		...
		if (enabled) {//if subject is going to happen
			addBlock(beginTime.getDay() - 1, startMin, length, i, name, location);
...
function addBlock(x, y, l, blockName, subjectName, location) {
	var fullWidth = Math.max(document.body.scrollWidth, document.documentElement.scrollWidth, document.body.offsetWidth, document.documentElement.offsetWidth, document.documentElement.clientWidth);
	var sceduleHeightPerMinute = document.getElementById("scedule").clientHeight / 465;
	var newWidth = document.getElementById("pauze").clientWidth;
	...
	block.style.top = div1.clientHeight + sceduleHeightPerMinute * y + "px";
